...
public static void searchStudent(String id, JTable table) {
    ...
    public static int update(int id, String gender, String fullname, String address, String email,
            String faculty, String studentPhone, String parentname,
            String parentaddress, String parentphonenum) {
        int rows = 0;
        try (Connection con = DBConnection.getConnection(); PreparedStatement pst = con.prepareStatement(
                "UPDATE students SET gender=?, fullname=?, address=?, email=?, faculty=?, "
                + "studentPhone=?, parentname=?, parentaddress=?, parentphonenum=? WHERE id=?")) {
            ...
            pst.setString(8, parentaddress);
            pst.setString(9, parentphonenum);
            pst.setInt(10, id);
